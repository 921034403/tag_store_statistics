...
# reply_type_id = 7 公众号
# reply_type_id = 8 H5
def statistic(store_lis = None,func = None):
    ...
    for store_info in store_lis:
        ...
        sql = "select reply_type_id,from_wechat_user_id, operation_type,from_wechat_user_id," \
              "DATE_FORMAT(operation_time,%s) as cycle " \
              "from v_praise_statistic " \
              "where store_id =%s "
        params = ['%Y-%m-%d', store_id]
        if start:
        ...
        res = cursor.fetchall()
        if res:
            sql = "select from_wechat_user_id,DATE_FORMAT(operation_time,%s) as add_time_desc," \
            ...
            func(res, add_time, store_name, kdt_id, cycle_lis,start)
...
# 统计每天互动概况
def praise_statistics1(res,add_time,store_name,kdt_id,cycle_lis,start):
    ...
    for action in res:
        ...
        operation_type = action["reply_type_id"]
        cycle = action['cycle']
        if reply_type_id == 8:
            if operation_type == 0:
...
def praise_statistics2(res,add_time,store_name,kdt_id,cycle_lis,start):
    ...
    for action in res:
        ...
        if reply_type_id == 8:
            ...
            cycle_infos[from_wechat_user_id]["hf_msg_count"].append(from_wechat_user_id)
        elif reply_type_id == 7:
            if operation_type > 0:
                cycle_infos[from_wechat_user_id]["wechat_click_count"] += 1
            cycle_infos[from_wechat_user_id]["wechat_msg_count"].append(from_wechat_user_id)
    # 结果入库前做整理
    statistics_result = []
    for cycle, cycle_info in cycle_infos.items():
    ...
    sql = "select reply_type_id,from_wechat_user_id,DATE_FORMAT(add_time,%s) as add_time_desc  " \
          "from third_part_wechat_praisechatrecord " \
          "where reply_msg is not null and store_id= %s "
    params = ['%Y-%m-%d',kdt_id]
    if start:

        sql += "and DATE_FORMAT(add_time,%s)>%s "
        params += ['%Y-%m-%d',start]
    cursor.execute(sql, params)
    res = cursor.fetchall()
    for action in res:
        ...
        if cycle_infos.get(from_wechat_user_id):
            if reply_type_id == 8:
                cycle_infos[from_wechat_user_id]["hf_msg_count"] += 1
            elif reply_type_id == 7:
                cycle_infos[from_wechat_user_id]["wechat_msg_count"] +=1
        else:
            cycle_infos[from_wechat_user_id] = {"from_wechat_user_id":i["from_wechat_user_id"],
                                "add_time_desc": i['add_time_desc'],
                                "add_time": add_time,
                                "store_name": store_name,
                                "kdt_id": kdt_id,
...
if __name__ == '__main__':
    statistic(store_lis=None,func=praise_statistics1)
    statistic(store_lis=None, func=praise_statistics2)